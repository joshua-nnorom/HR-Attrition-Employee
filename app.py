...
with tab5:
    ...
    st.write(
        "Click the button below to switch to the main HR Attrition application using both models.")
    st.link_button("🚀 Open Attrition Main App",
                   "https://hr-attrition-employee-predictions.streamlit.app")
# ─────────────────────────────────────────────
# Collect inputs dict
# ─────────────────────────────────────────────
inputs = {
...
with tab1:
    if load_error:
        ...
        **To run predictions:**
        ...
        risk_label = "High" if risk_pct >= 60 else (
            "Medium" if risk_pct >= 30 else "Low")
        badge_class = {"High": "badge-high",
                       "Medium": "badge-medium", "Low": "badge-low"}[risk_label]
        ...
        st.markdown(
            "<div class='section-title'>Employee Profile Summary</div>", unsafe_allow_html=True)
        ...
        # ── Risk Factor Radar ──
        st.markdown(
            "<div class='section-title'>Risk Factor Radar</div>", unsafe_allow_html=True)
        ...
        # ── Recommendations ──
        st.markdown(
            "<div class='section-title'> HR Recommendations</div>", unsafe_allow_html=True)
        ...
        for rec in recs_sorted:
            ...
            badge_text = {"urgent": "🔴 Urgent",
                          "medium": "🟡 Medium", "low": "🟢 Low"}[rec["priority"]]
            st.markdown(f"""
            <div class="{css_class}">
...
with tab2:
    ...
    m2.metric("Attrition Count", f"{attrited}",
              delta=f"{attr_rate}% rate", delta_color="inverse")
    ...
    with row1_c1:
        ...
        dept_attr = demo_df.groupby(
            ["Department", "Attrition"]).size().reset_index(name="Count")
    ...
    with row2_c2:
        ...
        fig_inc = px.box(demo_df, x="Attrition", y="MonthlyIncome", color="Attrition",
                         ...
                         template="plotly_white")
        fig_inc.update_layout(height=320, margin=dict(
            t=40, b=20), showlegend=False)
    ...
    with row3_c1:
        # Overtime
        ovt_data = demo_df.groupby(
            ["OverTime", "Attrition"]).size().reset_index(name="Count")
    ...
    st.markdown("<div class='section-title'>Satisfaction Metrics Heatmap</div>",
                unsafe_allow_html=True)
    sat_means = demo_df.groupby("Department")[
...
with tab3:
    ...
    inp_df = pd.DataFrame(list(numeric_inputs.items()), columns=[
                          "Metric", "Score (Normalised %)"])
...
# ══════════════════════════════════════════════
with tab4:
    st.markdown("<div class='section-title'> About This App & Model</div>",
                unsafe_allow_html=True)
